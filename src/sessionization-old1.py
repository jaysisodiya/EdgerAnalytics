...
with open(weblogfile_path, 'r') as fweblog:
    ...
    with open(outputfile_path, 'w') as foutfile:
        ...
        for log_line in fweblog:
            ...
            # we need to purge for inactive sessions
...
#                        sess_begtime = ip_dict[each_ip][0] + ' ' + ip_dict[each_ip][1]
#                        sess_duration = getSessDuration(sess_begtime, sess_lastime)
#                        sess_requests = ip_dict[each_ip][4]
#
#                        outstr = each_ip + col_delimiter + sess_begtime + col_delimiter + \
#                                sess_lastime + col_delimiter + str(sess_duration) + col_delimiter \
#                                + str(sess_requests) + '\n'
#
#                        print(outstr)
#                        foutfile.write(outstr)
                ...
                print("Removing key ", remove_keys)
                print("Removing sess ", remove_sess)
                inactiv_sess.clear()
            ...
            if curr_datetime in datetime_dict:
                datetime_dict[curr_datetime].append(curr_ip)
            else:
                datetime_dict[curr_datetime] = [curr_ip]
